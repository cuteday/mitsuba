                    ...
                    case ESpatialFilter::EStochasticBox:
                        {
                            ...
                            //Point origin = sdTree.aabb().clip(ray.o + offset);    // corrputed for unknown reasons
                            const AABB& sdAabb = sdTree.aabb();
                            Point origin = ray.o + offset;
                            for (int i = 0; i < Point::dim; i++) {
                                origin[i] = std::max(sdAabb.min[i], std::min(sdAabb.max[i], origin[i]));
                            }
